...
import {
  ...
  UserPlus, Trash2, Loader2, AlertCircle, CheckCircle2,
  Map, Chrome, Sparkles, ChevronDown
} from "lucide-react";
  ...
  const handleScrape = async () => {
    ...
    try {
      ...
      if (data.leads?.length === 0) {
        ...
        toast({ title: `Found ${data.leads.length} leads!`, description: "Review and import the ones you want." });
      }
    } catch (err: any) {
      if (err.message?.includes("SERPAPI_KEY") || err.message?.includes("setupRequired")) {
